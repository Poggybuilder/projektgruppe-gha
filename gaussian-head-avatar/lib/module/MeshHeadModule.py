...
class MeshHeadModule(nn.Module):
    ...
    def reconstruct(self, data):
        ...
        sdf = sdf.permute(0, 2, 1) + 0.1
        #print(f"\t{sdf.shape}", end="")
        ...
        #print(f"\n\nexp_color_input [{exp_color_input.shape}]:\n{exp_color_input}")
        ...
        # ======================= VERY TMP ==========================================================
        frequency = 128
        if self.global_counter % frequency == 0:
            for idx, datum in enumerate(data['verts_list']):
                output_path = f"/data/student_barth/TMP/MeshHead/{str(idx).zfill(4)}"
                makedirs(output_path, exist_ok=True)
                np.save(f"{output_path}/verts_{str(int(self.global_counter / frequency)).zfill(5)}.npy", datum.detach().cpu())
    ...
    def pre_train_sphere(self, iter, device):
        ...
        np.save("/data/student_barth/TMP/sphere.npy", data["query_pts"].cpu())
        print("==== The sphere was successfully pre-trained")
